...
const VerifiedContractsCounters = () => {
  ...
  if (
    !(isStatsFeatureEnabled ? countersStatsQuery.data : countersApiQuery.data)
  ) {
    return null;
  }

  const isLoading = isStatsFeatureEnabled ?
    countersStatsQuery.isPlaceholderData :
    countersApiQuery.isPlaceholderData;

  const contractsCount = isStatsFeatureEnabled ?
    countersStatsQuery.data?.total_contracts?.value :
    countersApiQuery.data?.smart_contracts;
  const newContractsCount = isStatsFeatureEnabled ?
    countersStatsQuery.data?.new_contracts_24h?.value :
    countersApiQuery.data?.new_smart_contracts_24h;
  ...
  return (
    ...
    >
      <StatsWidget
        ...
        // href={ config.features.stats.isEnabled ? { pathname: '/stats', query: { id: 'contractsGrowth' } } : undefined }
      />
      <StatsWidget
        ...
        href={
          config.features.stats.isEnabled ?
            { pathname: '/stats', query: { id: 'verifiedContractsGrowth' } } :
            undefined
        }
      />
    </Box>
